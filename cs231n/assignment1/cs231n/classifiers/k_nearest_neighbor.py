...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in list(range(num_test)):
      for j in list(range(num_train)):
        ...
        #dists[i, j] = np.sum(np.abs(np.subtract(self.X_train[j], X[i]))) // L1 distance
  ...
  def compute_distances_no_loops(self, X):
    ...
    num_train = self.X_train.shape[0]
    dists = np.zeros((num_test, num_train))
    prod = -2 * np.dot(self.X_train,X.T)
    aSquared = np.sum(np.power(self.X_train,2),1)
    bSquared = np.sum(np.power(X,2),1)
    sum1 = prod.T + aSquared.T
    sum1 = sum1.T + bSquared
    dists = np.sqrt(sum1)
